...
class APCCheckpointManager(CheckpointManager):
    @staticmethod
    @retry
    def get_sentiment_classifier(checkpoint: str = None, **kwargs):
        """
    ...
    def get_checkpoint(checkpoint: str = "multilingual"):
        ...
        if checkpoint.lower() in [
            k.lower() for k in aspect_sentiment_classification_checkpoint.keys()
        ]:
...
class ATEPCCheckpointManager(CheckpointManager):
    @staticmethod
    @retry
    def get_aspect_extractor(checkpoint: str = None, **kwargs):
        """
...
class TCCheckpointManager(CheckpointManager):
    @staticmethod
    @retry
    def get_text_classifier(checkpoint: str = None, **kwargs):
        """
    ...
    def get_checkpoint(checkpoint: str = "multilingual"):
        ...
        if checkpoint.lower() in [
            k.lower() for k in text_classification_checkpoint.keys()
        ]:
            print(colored("Downloading checkpoint:{} ...".format(checkpoint), "green"))
        else:
            print(colored("Checkpoint:{} is not found.".format(checkpoint), "red"))
...
class TADCheckpointManager(CheckpointManager):
    ...
    def get_checkpoint(checkpoint: str = "multilingual"):
        ...
        if checkpoint.lower() in [
            k.lower() for k in tad_classification_checkpoint.keys()
        ]:
            print(colored("Downloading checkpoint:{} ...".format(checkpoint), "green"))
        else:
            print(colored("Checkpoint:{} is not found.".format(checkpoint), "red"))
...
def available_checkpoints(task="", show_ckpts=False):
    try:
            ...
            if max_ver == "N.A." or StrictVersion(min_ver) <= StrictVersion(
                ...
                if task:
                    t_checkpoint_map.update(
                        checkpoint_map[c_version][task.upper()]
                        if task.upper() in checkpoint_map[c_version]
                        else {}
                    )
                    if show_ckpts:
        ...
        print(
            colored(
                "There may be some checkpoints available for early versions of PyABSA, see {}".format(
                    task, __version__, checkpoint_url
                ),
                "yellow",
            )
        )
