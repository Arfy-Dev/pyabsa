...
class Trainer:
    def __init__(
        ...
        if isinstance(config, APCConfigManager):
            ...
            self.model_class = TADTextClassifier
            self.task = "text_defense"
        ...
        if "MV" not in self.config.args:
            self.config.MV = MetricVisualizer(
                name=config.model.__name__ + "-" + self.config.dataset_name,
        ...
        self.from_checkpoint = (
            findfile.find_dir(os.getcwd(), from_checkpoint) if from_checkpoint else ""
        )
    ...
    def _run(self):
        ...
        for i, s in enumerate(seeds):
            ...
            if self.checkpoint_save_mode:
                model_path.append(
                    self.train_func(self.config, self.from_checkpoint, self.logger)
                )
            else:
                # always return the last trained model if dont save trained model
                model = self.model_class(
                    model_arg=self.train_func(
                        self.config, self.from_checkpoint, self.logger
                    )
                )
    ...
    def load_trained_model(self):
        if not self.inference_model:
            print(
                "No trained model found, this could happen while training only using training set."
            )
