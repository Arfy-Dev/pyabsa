        ...
        if len(s) > 0:
            ...
            if len(Labels) > 3:
                ...
                for p_idx in range(len(p) - 1):
                    if (
                        p[p_idx] != p[p_idx + 1]
                        and p[p_idx] != str(SENTIMENT_PADDING)
                        and p[p_idx + 1] != str(SENTIMENT_PADDING)
                    ) or (
                        p[p_idx] != str(SENTIMENT_PADDING)
                        and p[p_idx + 1] == str(SENTIMENT_PADDING)
                    ):
                ...
                for t_idx in range(1, len(t)):
                    ...
                    if p[t_idx - 1] != str(SENTIMENT_PADDING) and split_aspect(
                        t[t_idx - 1], t[t_idx]
                    ):
...
def convert_examples_to_features(examples, max_seq_len, tokenizer, opt=None):
    ...
    for ex_index, example in enumerate(
        tqdm.tqdm(examples, postfix="convert examples to features")
    ):
        ...
        if (
            polarity != SENTIMENT_PADDING or int(polarity) != SENTIMENT_PADDING
