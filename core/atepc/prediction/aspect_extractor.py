...
class AspectExtractor:
        ...
        if not isinstance(model_arg, str):
            ...
            try:
                state_dict_path = find_file(
                    model_arg, ".state_dict", exclude_key=["__MACOSX"]
                )
                model_path = find_file(model_arg, ".model", exclude_key=["__MACOSX"])
                tokenizer_path = find_file(
                    model_arg, ".tokenizer", exclude_key=["__MACOSX"]
                )
                ...
                self.tokenizer.bos_token = (
                    self.tokenizer.bos_token if self.tokenizer.bos_token else "[CLS]"
                )
                self.tokenizer.eos_token = (
                    self.tokenizer.eos_token if self.tokenizer.eos_token else "[SEP]"
                )
    ...
    def extract_aspect(
        self, inference_source, save_result=True, print_result=True, pred_sentiment=True
    ):
    ...
    def _extract(self, examples, infer_batch_size=256):
        ...
        self.infer_dataloader = DataLoader(
            ...
            pin_memory=True,
            batch_size=self.opt.infer_batch_size,
        )
        ...
        for i_batch, (
            ...
            for i, i_ate_logits in enumerate(ate_logits):
                pred_iobs = []
                sentence_res.append(
                    (all_tokens[i + (self.opt.infer_batch_size * i_batch)], pred_iobs)
                )
                for j, m in enumerate(label_ids[i]):
                    if j == 0:
                        continue
                    elif len(pred_iobs) == len(
                        all_tokens[i + (self.opt.infer_batch_size * i_batch)]
                    ):
                        break
                    else:
                ...
                for idx in range(1, len(polarity)):
                    if polarity[idx - 1] != str(SENTIMENT_PADDING) and split_aspect(
                    ...
                    if (
                        polarity[idx] != str(SENTIMENT_PADDING)
    ...
    def _infer(self, examples):
        ...
        if len(infer_data) >= 100:
            it = tqdm.tqdm(
                self.infer_dataloader, postfix="classifying aspect sentiments..."
            )
        else:
            it = self.infer_dataloader
        for i_batch, batch in enumerate(it):
            ...
            with torch.no_grad():
                ...
                for i, i_apc_logits in enumerate(apc_logits):
                    if (
                        "index_to_label" in self.opt.args
                        and int(i_apc_logits.argmax(axis=-1)) in self.opt.index_to_label
                    ):
                        sent = self.opt.index_to_label.get(
