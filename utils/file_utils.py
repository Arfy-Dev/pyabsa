...
def generate_inference_set_for_apc(dataset_path):
    ...
    train_datasets = find_files(
        os.getcwd(),
        ["dataset", "train", "apc", dataset_name],
        exclude_key=[".inference", "readme"],
    )
    valid_datasets = find_files(
        os.getcwd(),
        ["dataset", "valid", "apc", dataset_name],
        exclude_key=[".inference", "readme"],
    )
    test_datasets = find_files(
        os.getcwd(),
        ["dataset", "test", "apc", dataset_name],
        exclude_key=[".inference", "readme"],
    )
        ...
        try:
            ...
            for i in range(0, len(lines), 3):
                sample = (
                    lines[i]
                    .strip()
                    .replace("$T$", "[ASP]{}[ASP]".format(lines[i + 1].strip()))
                )
...
def convert_apc_set_to_atepc_set(path, use_tokenizer=False):
    ...
    if os.path.isfile(path):
        ...
        files = find_files(
            path, ["dataset", "apc"], exclude_key=[".inference", "readme"]
        )
    else:
...
def save_model(opt, model, tokenizer, save_path):
    ...
    if opt.save_mode == 1 or opt.save_mode == 2:
        ...
        if opt.save_mode == 1:
            torch.save(
                model_to_save.state_dict(), save_path + opt.model_name + ".state_dict"
            )  # save the state dict
        elif opt.save_mode == 2:
            torch.save(
                model.cpu(), save_path + opt.model_name + ".model"
            )  # save the state dict
...
def validate_datasets_version():  # retry_count is for unstable conn to GitHub
    try:
        ...
        print(
            "Remote ABSADataset version: {} Local ABSADatasets version: {}".format(
